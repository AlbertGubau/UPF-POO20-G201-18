...
public class Administrator extends User{ //Administrator es una clase hija de User por lo tanto usamos la keyword extends
    ...
    public Boolean manageAuction(AuctionItem a, String date){ //Método que permite manejar la subasta siempre que nos encontremos en una fehc vàlida, i.e, que no hayamos pasado el deadline                                                          
        
        if(!a.frozen(date)){
            System.out.println(getName() + " is managing the item " + a.getName() + " at " + date + "."); //Imprime por pantalla quien esta a carga de la subasta y devuelve true si estamos en una fecha válida
            return true;
        }
